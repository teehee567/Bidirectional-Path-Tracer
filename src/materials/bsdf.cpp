...
namespace bsdf {
    ...
    float GTR1(float absDotHL, float a) {
        ...
        float a2 = a * a;
        return (a2 - 1.0f) / (kPi * std::log(a2) * (1.0f + (a2 - 1.0f) * absDotHL * absDotHL));
    }
    ...
    float GgxAnisotropicD(const glm::vec3& wm, float ax, float ay){
        ...
        return 1.0f / (kPi * ax * ay * pow(dotHX2 / ax2 + dotHY2 / ay2 + cos2Theta, 2));
    }
    ...
    glm::vec3 SampleGgxVndfAnisotropic(const glm::vec3& wo, float ax, float ay, float u1, float u2){
        ...
        float r = sqrt(u1);
        float phi = (u2 < a) ? (u2 / a) * kPi : kPi + (u2 - a) / (1.0f - a) * kPi;
        float p1 = r * cos(phi);
    ...
    float EvaluateDisneyDiffuse(const SurfaceParameters& surface, const glm::vec3& wo, const glm::vec3& wm, const glm::vec3& wi, bool thin){
        ...
        return kInvPi * (retro + subsurfaceApprox * (1.0f - 0.5f * fl) * (1.0f - 0.5f * fv));
    }
    ...
    bool SampleDisneySpecular(const SurfaceParameters& surface, glm::vec3 v, BsdfSample& sample){
        ...
        sample.reflectance = EvaluateDisneyBRDF(surface, wo, wm, wi, sample.forwardPdfW, sample.reversePdfW);
        sample.wi = glm::normalize(surface.basis.transform * wi);
        return sample.forwardPdfW > 0.0f;
    }
