#ifndef MTL_GGX_BXDF_H
#define MTL_GGX_BXDF_H

#include "../bxdf_utils.h"

class MtlGGX : public BxDF {
  public:
    MtlGGX(shared_ptr<texture> diffuse,
      ...
      : diffuse_tex(diffuse),
        ...
        rough(std::clamp(roughness, 1e-4, 1.0)),
        ior(ior),
        dissolve(std::clamp(dissolve, 0.0, 1.0)) {}

  private:
    Spectrum evaluate(const Vec2 &texPos, const Vec3 &i, const Vec3 &o) const override {
        color specTex = sample_texture(specular_tex, texPos);

        if (i.z() * o.z() > 0) {
            Vec3 n = normalize_or_default(i + o);
            Spectrum spec = F(texPos, i, o, n) * D(n) * G(i, o, n) / std::max(1e-6, 4.0 * std::abs(i.z() * o.z()));
            Spectrum diff = sample_texture(diffuse_tex, texPos) / pi * dissolve;
            return spec + diff;
        } else {
            Vec3 n = normalize_or_default(o + i * ((o.z() > 0) ? ior : (1.0 / ior)));
            Spectrum trans = (color(1,1,1) - F(texPos, i, o, n)) * D(n) * G(i, o, n);
    ...
    Spectrum sample(const Vec2 &texPos, const Vec3 &i, Vec3 &o, double &pdf, Sampler &RNG) const override {
        ...
        double prob = (rough + spectrum_norm(fres)) / (1.0 + 2.0 * rough);
        ...
        if (xi < prob) {
            ...
            Spectrum diff = sample_texture(diffuse_tex, texPos) / pi;
    ...
    double evaluateImportance(const Vec2 &texPos, const Vec3 &i, const Vec3 &o) const override {
        Spectrum fres = F(texPos, i, o, normalize_or_default(i + o));
        double prob = (rough + spectrum_norm(fres)) / (1.0 + 2.0 * rough);

        if (i.z() * o.z() > 0) {
            Vec3 n = normalize_or_default(i + o);
            return D(n) * prob + (1.0 / (2.0 * pi)) * (1.0 - prob) * dissolve;
        } else {
            Vec3 n = normalize_or_default(o + i * ((o.z() > 0) ? ior : (1.0 / ior)));
            return D(n) * (1.0 - prob) * (1.0 - dissolve);
        }
    }

    Spectrum F(const Vec2 &texPos, const Vec3 &i, const Vec3 &o, const Vec3 &n) const {
        Spectrum spec = sample_texture(specular_tex, texPos);
        if (o.z() > 0) {
            double cos_theta = std::abs(dot(o, n));
            return spec + (color(1,1,1) - spec) * std::pow(1.0 - cos_theta, 5.0);
        } else {
